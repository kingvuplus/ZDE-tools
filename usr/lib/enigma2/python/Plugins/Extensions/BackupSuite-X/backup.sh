...
MY_MOUNT=$2
UBIFS="$(df -h $MY_MOUNT | grep ubi0:rootfs | awk {'print $1'})" > /dev/null 2>&1
if [ "$UBIFS" = ubi0:rootfs ] ; then
	HDD_MOUNT="$(ls -l $MY_MOUNT | grep -o media/x)"
		if [ "$?" = "0" ] ; then
	...
	touch $MY_MOUNT/hdd-check > /dev/null 2>&1
fi
if [ -f $MY_MOUNT/hdd-check ] ; then  
	CHECKMOUNT1="$(df -h $MY_MOUNT | tail -n 1 | awk {'print $6'})"
	CHECKMOUNT2="$(df -h $MY_MOUNT | tail -n 1 | awk {'print $5'})"
	if [ "${CHECKMOUNT1:1:5}" = media ] ; then
		TOTALSIZE="$(df -h $MY_MOUNT | tail -n 1 | awk {'print $2'})"
		FREESIZE="$(df -h $MY_MOUNT | tail -n 1 | awk {'print $4'})"	
		MEDIA="$(df -h $MY_MOUNT | tail -n 1 | awk {'print $6'})"
	elif [ "${CHECKMOUNT2:1:5}" = media ] ; then
		TOTALSIZE="$(df -h $MY_MOUNT | tail -n 1 | awk {'print $1'})"		
		FREESIZE="$(df -h $MY_MOUNT | tail -n 1 | awk {'print $3'})"	
		MEDIA="$(df -h $MY_MOUNT | tail -n 1 | awk {'print $5'})"
	else
		TOTALSIZE="??"
		FREESIZE="??"
		MEDIA="unknown"
	fi
	echo -n " -> $MY_MOUNT -> $MEDIA ($TOTALSIZE, "; $SHOW "message16" ; echo "$FREESIZE)"
	echo -n $WHITE
	backupsuite.sh $MY_MOUNT
	rm -f $MY_MOUNT/hdd-check
	sync
else
